...
{
    display.framebuffer = (Framebuffer*)malloc(sizeof(Framebuffer));
    memcpy(display.framebuffer, framebuffer, sizeof(Framebuffer));
...
int Display::putc(char ch) 
{
    ...
    if(to_put == 1)
    {
        putchar_at(framebuffer, ch, x, y);
    }

    if(to_put == 2)
    {
        empty_char_at(framebuffer, x - 1, y);
    }
